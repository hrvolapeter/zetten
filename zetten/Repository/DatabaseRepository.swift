...
class DatabaseRepository {
    ...
    private func handleChange(notes: [Note]) {
        ...
        try! dbQueue.write { db in
            ...
            logger.debug("Removing \(localNotesSet.count) notes not present in Firestore update")
    ...
    func fetch(query q: String) throws -> DatabasePublishers.Value<[Note]> {
        logger.debug("Query local db with: \(q)")
        if q.isEmpty {
            let observation = ValueObservation.tracking { db in
                try Note.order(sql: "createdTime DESC").fetchAll(db)
            }
        ...
        WHERE note_ft MATCH '{title tags content}: "' || ? || '"' AND rank MATCH 'bm25(10.0, 5.0, 1.0)'
        ORDER BY rank, createdTime DESC
        """,
                arguments: [q])
    ...
    func fetchAll(query q: String) throws -> [Note] {
    ...
    func fetch_all_tags() throws -> Set<String> {
        try dbQueue.read { db in
            let stmt = try db.makeSelectStatement(sql: "SELECT tags FROM note")
            let tags_array = try String.fetchAll(stmt)
            let decoder = JSONDecoder()
            let tags = tags_array.flatMap { try! decoder.decode(Array<String>.self, from: $0.data(using: .utf8)!) }
            return Set(tags)
        }
    }
